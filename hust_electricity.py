...
#pattern for finding 
pattern1 = re.compile(r'<input name="TextBox3".*?type="text".*?value="(.*)".*?readonly="readonly".*?id="TextBox3".*?/> ',re.S)
...
class hust_query():
    ...
    def get_info_step1(self,programId):
    ...
    def get_info_step2(self,programId,txtyq):
        info = self.get_info_step1(programId)
        data={
           " __EVENTTARGET":"txtyq",
           " __EVENTARGUMENT":"",
           " __LASTFOCUS":"",
    ...
    #get the result
    def get_result(self):
        info = self.get_info_step2(self._programId,self._txtyq)
        data={
    "__EVENTTARGET":"",
    "__EVENTARGUMENT":"",
    "__LASTFOCUS":"",
    ...
    #handle the result from get
    def handle_result(html_data):
        match1 = pattern1.search(html_data)
...
if __name__=='__main__':
    query=hust_query(args.programId,args.txtyq,args.txtld,args.txtroom)
    html_data=query.get_result()
    hust_query.handle_result(html_data)
